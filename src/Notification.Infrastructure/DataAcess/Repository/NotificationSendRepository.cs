...
public class NotificationSendRepository : INotificationSendRepository 
{
    ...
    public async Task CreateAsync(NotificationSend request) 
    {
        ...
        await _db.NotificationSents.AddAsync(request);
    }

    public async Task<ICollection<NotificationSend?>> GetAllAsync(string query) 
    {
        IQueryable<NotificationSend?> notificationsents = _db.NotificationSents.Include(s=>s.notificationTemplate).Include(s=>s.notificationDevice).Where(s=>s.Status== (int)NotificationStatus.Active);
    ...
    {
        return await _db.NotificationSents.Include(u=>u.notificationTemplate).Include(u=>u.notificationDevice).SingleOrDefaultAsync(u => u.Id == Id && u.Status== (int)NotificationStatus.Active);
    }
}
