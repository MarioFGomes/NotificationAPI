...
public class GetByIdNotificationTypesQueryHandler : IRequestHandler<GetByIdNotificationTypesQuery, ResponseNotificationTypes> 
{
    ...
    {
        var notificationTypes = await _notificationTypeRepository.GetbyIdAsync(request.Id)?? throw new GenericErrorException(ResourceErrorMessages.NotificationNotFound);
